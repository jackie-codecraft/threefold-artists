                        ...
                        <a href="{{ route('gallery', ['related_type' => 'blog_post', 'related_id' => $post->id]) }}" class="group block">
                            <div class="aspect-[4/3] overflow-hidden bg-linen">
                                @if($galleryItem->getFirstMediaUrl('media'))
                                    ...
                                    @else
                                        <img src="{{ $galleryItem->thumbnailUrl() }}" alt="{{ $galleryItem->title ?? 'Blog gallery image' }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                    @endif
                                @else
