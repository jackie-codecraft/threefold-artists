<x-layouts.app title="Become a Founding Supporter" metaDescription="Pledge your future support for Threefold Artists and help keep theatre, music, dance, and the arts accessible.">
    ...
    {{-- Pledge form --}}
    <section class="bg-curtain-red py-16 sm:py-24" x-data="{ selectedAmount: '{{ old('amount', '100') }}', customAmount: '', useCustom: false }">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-3xl bg-white p-6 shadow-2xl shadow-black/25 sm:p-10 lg:p-14">
                ...
                <form action="{{ route('pledge.store') }}" method="POST" class="mt-10 space-y-9">
                    ...
                    <section>
                        <h3 class="mb-5 text-xl font-semibold text-curtain-red">Contact Information</h3>
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div>
                                <label for="name" class="sr-only">Full name</label>
                                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Full Name" class="w-full rounded-xl border border-curtain-red/25 bg-white px-4 py-3 text-theatre-black placeholder-gray-400 focus:border-curtain-red focus:ring-curtain-red">
                                @error('name')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label for="email" class="sr-only">Email address</label>
                                <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="Email Address" class="w-full rounded-xl border border-curtain-red/25 bg-white px-4 py-3 text-theatre-black placeholder-gray-400 focus:border-curtain-red focus:ring-curtain-red">
                                @error('email')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div class="sm:col-span-2">
                                <label for="phone" class="sr-only">Phone number</label>
                                <input type="tel" name="phone" id="phone" value="{{ old('phone') }}" placeholder="Phone Number (Optional)" class="w-full rounded-xl border border-curtain-red/25 bg-white px-4 py-3 text-theatre-black placeholder-gray-400 focus:border-curtain-red focus:ring-curtain-red">
                    ...
                    <section>
                        <h3 class="mb-5 text-xl font-semibold text-curtain-red">Pledge Amount</h3>
                        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                            @foreach([25, 50, 100, 250, 500] as $amount)
                                <label class="cursor-pointer rounded-xl border border-curtain-red/25 px-4 py-4 text-center transition hover:border-curtain-red hover:bg-linen" :class="selectedAmount == '{{ $amount }}' && ! useCustom ? 'border-curtain-red bg-linen ring-2 ring-curtain-red/20' : ''">
                            ...
                            @endforeach
                            <label class="cursor-pointer rounded-xl border border-curtain-red/25 px-4 py-4 text-center transition hover:border-curtain-red hover:bg-linen" :class="useCustom ? 'border-curtain-red bg-linen ring-2 ring-curtain-red/20' : ''">
                        ...
                        <div class="mt-4" x-show="useCustom" x-cloak>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="custom_amount" min="1" step="1" x-model="customAmount" x-on:input="selectedAmount = customAmount" placeholder="Custom amount" class="w-full rounded-xl border border-curtain-red/25 bg-white py-3 pl-8 pr-4 text-theatre-black placeholder-gray-400 focus:border-curtain-red focus:ring-curtain-red">
                            </div>
                        </div>
                    ...
                    <section>
                        <h3 class="mb-5 text-xl font-semibold text-curtain-red">Additional Message</h3>
                        <label for="message" class="sr-only">Additional message</label>
                        <textarea name="message" id="message" rows="6" placeholder="Share why the arts matter to you or how you heard about Threefold Artists." class="w-full rounded-xl border border-curtain-red/25 bg-white px-4 py-3 text-theatre-black placeholder-gray-400 focus:border-curtain-red focus:ring-curtain-red">{{ old('message') }}</textarea>
                        @error('message')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </section>

                    <div class="rounded-2xl border border-curtain-red/15 bg-curtain-red/5 p-5">
                        <label class="flex items-start gap-3 text-sm leading-6 text-gray-700">
                            <input type="checkbox" name="pledge_acknowledgment" value="1" required class="mt-1 h-4 w-4 rounded border-curtain-red/40 text-curtain-red focus:ring-curtain-red" @checked(old('pledge_acknowledgment'))>
                    ...
                    <div class="text-center">
                        <button type="submit" class="inline-flex items-center justify-center rounded-full bg-curtain-red px-10 py-4 text-sm font-semibold uppercase tracking-wide text-white shadow-lg shadow-curtain-red/25 transition hover:bg-theatre-black">
