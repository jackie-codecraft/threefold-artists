<x-layouts.app title="Reply to Contact Message">
    <section class="py-16 sm:py-24">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                ...
                <p class="text-gray-500 leading-relaxed mb-8">Your reply has been sent to {{ $contactMessage->name }} at {{ $contactMessage->email }}.</p>
            ...
            @else
                <h1 class="font-display text-3xl font-light text-theatre-black mb-2">Reply to {{ $contactMessage->name }}</h1>
                <p class="text-gray-400 text-sm mb-8">{{ $contactMessage->email }} &middot; {{ $contactMessage->created_at->format('M j, Y g:i A') }}</p>

                {{-- Original message --}}
                <div class="mb-8 p-5 bg-gray-50 border-l-4 border-gray-200">
                    @if($contactMessage->subject)
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-gray-400 mb-2">Subject</p>
                        <p class="text-theatre-black font-medium mb-3">{{ $contactMessage->subject }}</p>
                    @endif
                    <p class="text-xs font-semibold tracking-[0.15em] uppercase text-gray-400 mb-2">Message</p>
                ...
                @if($contactMessage->isReplied())
                    <div class="mb-8 p-5 bg-green-50 border-l-4 border-stage-gold">
                        <p class="text-xs font-semibold tracking-[0.15em] uppercase text-gray-400 mb-2">Previous Reply ({{ $contactMessage->replied_at->format('M j, Y g:i A') }})</p>
                        <p class="text-gray-600 leading-relaxed">{{ $contactMessage->reply }}</p>
                    </div>
                @endif
